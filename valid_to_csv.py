...
def extract_log(log_origin, thresh_value):
	...
	for line in f_origin:
		...
		xmin = int(float(line.split(' ')[2]) + 0.5)
		ymin = int(float(line.split(' ')[3]) + 0.5)
		xmax = int(float(line.split(' ')[4]) + 0.5)
		ymax = int(float(line.split(' ')[5]) + 0.5)
...
if __name__ == '__main__':
	extract_log('./result_2018_8_19.txt', 0.4)